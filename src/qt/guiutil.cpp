...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a NPay address (e.g. %1)").arg("Ng0iHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no NPay: URI
...
{
    // Convert npay:// to npay:
    //
    //    Cannot handle this later, because npay:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open npay.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NPay.lnk";
}
...
{
    return GetAutostartDir() / "npay.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a npay.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NPay\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the npay app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add npay app to startup item list
