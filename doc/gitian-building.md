...
*Setup instructions for a gitian build of NPay using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NPay
...
Clone the git repositories for npay and gitian and then checkout the npay version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the npay repository
...
To build NPay (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the npay repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/npay/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/KryptoniteOU/NPay
...
For example:
```bash
URL=https://github.com/KryptoniteOU/NPay.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit npay=${COMMIT} --url npay=${URL} ../NPay/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit npay=${COMMIT} --url npay=${URL} ../NPay/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit npay=${COMMIT} --url npay=${URL} ../NPay/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/npay-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/npay-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/npay-build.assert
```
...
[KryptoniteOU/gitian.sigs](https://github.com/KryptoniteOU/gitian.sigs/) repository, or if that's not possible to create a pull
request.
